...
const OWNER_ID = 1228723117; // Faqat shu ID ishlata oladi
...
function startBot() {
    ...
    bot.onText(/❌ Testni to‘xtatish/, (msg) => {
        ...
        delete userSessions[chatId];  // Sessiyani o'chiramiz — test to'xtaydi

        bot.sendMessage(chatId, "🛑 Test to‘xtatildi!", mainMenu);
    });

    // SO‘Z QO‘SHISH (tugma bosilganda)
    bot.onText(/➕ So‘z qo‘shish/, (msg) => {
        if (msg.chat.id !== OWNER_ID) return;
        bot.sendMessage(msg.chat.id, "✍ So‘z(lar)ni qo‘shing: Format `salom - 안녕하세요`\n\nBir nechta so‘z kiritsangiz, yangi qatordan yoki vergul bilan ajrating.", mainMenu);
    });
    ...
    bot.onText(/📄 Mening so‘zlarim/, async (msg) => {
        ...
        // Oldingi ro'yxat xabarini o'chirish (agar mavjud bo'lsa)
    ...
    bot.on('message', async (msg) => {
        ...
        // Agar test to'xtatilgan bo'lsa, javoblarni qabul qilmaslik uchun
        ...
        if (userSessions[chatId]?.editingId) {
            ...
            // So‘z yangilandi degan xabar va menyu yuborish
            await bot.sendMessage(chatId, "✅ So‘z yangilandi!", mainMenu);

            // Yangilangan so‘zlar ro‘yxatini ko‘rsatish uchun
            // Oldingi ro'yxat xabarini o'chirish
            if (userSessions[chatId]?.lastListMessageId) {
                try {
                    await bot.deleteMessage(chatId, userSessions[chatId].lastListMessageId);
                } catch {}
            ...
            userSessions[chatId].lastListMessageId = sentMsg.message_id;
        ...
        // Yangi so‘z qo‘shish
        if (text.includes('-')) {
            return await addMultipleWords(chatId, text, bot);
        }
    ...
    async function addMultipleWords(chatId, text, bot) {
        ...
        if (!entries.length) return bot.sendMessage(chatId, "❗ Format: salom - 안녕하세요", mainMenu);
    ...
    // TEST BOSHLASH funktsiyasi — 1, 2, 3 sonlari ketma-ket chiqadi, so‘ng test boshlanadi
